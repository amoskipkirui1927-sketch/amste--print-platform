// agent-script.js - Production Ready Admin Dashboard
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on(`notification_admin`, (notification) => {
        if (currentAgent) {
...
async function checkAgentLogin() {
    ...
    if (stored && token) {
        try {
            ...
            if (verify.valid && verify.role === 'admin') {
...
async function downloadFile(orderId, fileIndex) {
    window.open(`/api/orders/${orderId}/file/${fileIndex}?token=${currentToken}`, '_blank');
}
...
async function loadCustomerMessages() {
    ...
    container.innerHTML = convs.map(c => `
        <div class="conversation-item" onclick="selectConversation('${c.customerId}')" style="background: ${c.unreadCount > 0 ? '#e8f0fe' : 'transparent'};">
            ...
            <div><small>📞 ${c.customerPhone}</small></div>
        </div>
    `).join('');
}
...
async function loadUsersForBroadcast() {
    ...
    container.innerHTML = users.map(u => `
        <div class="user-checkbox-item">
            ...
            <label for="user_${u.id}">${u.firstName} ${u.lastName} (${u.email})</label>
        </div>
    `).join('');
}
    ...
    chatArea.innerHTML = `
        <div class="chat-window">
            ...
            <div class="chat-input-area">
                <textarea id="adminMessageInput" placeholder="Type message to ${conv.customerName}..."></textarea>
...
async function deleteAdminMessageForMe(messageId) {
    showConfirmDialog('Delete this message?', async (confirmed) => {
        if (!confirmed) return;
        await fetch(`/api/messages/${messageId}/for-me`, { 
            method: 'DELETE', 
