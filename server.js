// server.js - Production Ready Backend for Amste Print Media
...
const upload = multer({ 
    ...
    fileFilter: (req, file, cb) => {
        const allowedTypes = /jpeg|jpg|png|gif|bmp|pdf|doc|docx/;
...
function loadDB() {
    try {
        if (fs.existsSync(DB_PATH)) {
            ...
            if (!db.pricing) db.pricing = { blackWhite: 5, color: 10, imagePrint: 25 };
...
app.post('/api/admin/login', (req, res) => {
    ...
    if (email === process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD_HASH) {
        ...
        res.json({ success: true, agent: { id: 'admin1', name: 'Amos Kipkirui', email, role: 'super_admin' }, token });
    } else {
...
app.get('/api/verify-token', authenticateToken, (req, res) => {
    if (req.user.role === 'admin') {
        ...
        if (user && !user.suspended) {
            ...
            res.json({ valid: true, role: 'customer', user: safeUser });
        } else {
            res.status(401).json({ valid: false, error: 'User account issue' });
        }
    }
});

// ---------- Profit Analytics Endpoint ----------
app.get('/api/admin/profit-analytics', authenticateToken, (req, res) => {
...
app.post('/api/orders', authenticateToken, upload.array('files', 10), (req, res) => {
    try {
        const { settings } = JSON.parse(req.body.orderData);
        ...
        addNotification(req.user.id, notificationMsg, 'order');
...
// ---------- Admin Routes ----------
app.get('/api/admin/orders/pending', authenticateToken, (req, res) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Access denied' });
...
function calculateOrderCost(order, pageCount) {
    ...
    const totalCost = printingCost + imageCost;
    return totalCost;
}

app.post('/api/admin/orders/:orderId/confirm-pages', authenticateToken, (req, res) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Access denied' });
...
app.put('/api/admin/orders/:orderId/status', authenticateToken, (req, res) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Access denied' });
...
app.get('/api/orders/:orderId/file/:fileIndex', authenticateToken, (req, res) => {
    const order = db.orders.find(o => o.id === req.params.orderId);
    if (!order) return res.status(404).json({ error: 'Order not found' });
...
// ---------- Messaging Routes ----------
app.get('/api/users/:userId/messages', authenticateToken, (req, res) => {
    if (req.user.id !== req.params.userId && req.user.role !== 'admin') {
        return res.status(403).json({ error: 'Access denied' });
    }
...
app.post('/api/messages', authenticateToken, (req, res) => {
    ...
    if (recipientType === 'customer') {
        addNotification(recipientId, `New message from ${message.senderType === 'admin' ? 'Support Team' : 'Customer'}`, 'message');
    }
...
app.delete('/api/messages/:messageId/for-me', authenticateToken, (req, res) => {
    const msg = db.messages.find(m => m.id === req.params.messageId);
    if (msg && (msg.senderId === req.user.id || msg.recipientId === req.user.id)) {
        if (!msg.deletedBy.includes(req.user.id)) {
            msg.deletedBy.push(req.user.id);
            saveDB();
...
// ---------- Notifications ----------
app.get('/api/users/:userId/notifications', authenticateToken, (req, res) => {
    if (req.user.id !== req.params.userId && req.user.role !== 'admin') {
        return res.status(403).json({ error: 'Access denied' });
    }
...
app.post('/api/admin/pricing', authenticateToken, (req, res) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Access denied' });
...
app.put('/api/admin/users/:userId/toggle-suspend', authenticateToken, (req, res) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Access denied' });
...
// ---------- Unattended tasks count ----------
app.get('/api/admin/unattended-count', authenticateToken, (req, res) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Access denied' });
...
server.listen(PORT, () => {
    ...
    console.log(`🔧 Admin portal: http://localhost:${PORT}/agent`);
});